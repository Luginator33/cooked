...
export default function Discover({ tasteProfile, initialTab }) {
  ...
  return (
    ...
    <div style={{ maxWidth:480, margin:"0 auto", minHeight:"100vh", background:C.cream, paddingBottom:90, position:"relative" }}>
        ...
        {(tab === "discover" || tab === "map") && (
          <div style={{ position:"relative", marginBottom:12 }}>
            <button onClick={() => setCityPickerOpen(v => !v)} style={{ display:"flex", alignItems:"center", gap:5, background:"none", border:"none", cursor:"pointer", padding:0, outline:"none" }}>
              <span style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:15, fontWeight:700, color:C.terracotta, fontStyle:"italic" }}>{city === "All" ? "All Cities" : city}</span>
      ...
      {/* Ask Tab */}
      {tab === "ask" && <ChatBot onClose={() => setTab("discover")} allRestaurants={allRestaurants} />}
      ...
      <div style={{ position:"fixed", bottom:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, background:"rgba(250,246,240,0.95)", backdropFilter:"blur(16px)", borderTop:`1px solid ${C.border}`, display:"flex", padding:"10px 0 20px", zIndex:100, isolation:"isolate" }}>
        ...
        <div onClick={() => setTab("profile")} style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:3, cursor:"pointer", padding:4 }}>
          <span style={{ width:24, height:24, borderRadius:"50%", overflow:"hidden", display:"inline-flex", alignItems:"center", justifyContent:"center", transform: tab==="profile" ? "scale(1.15)" : "scale(1)", transition:"transform 0.2s", border: tab==="profile" ? `2px solid ${C.terracotta}` : "2px solid transparent" }}>
            <img src={localStorage.getItem("cooked_profile_photo") || ""} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} onError={e => { e.target.style.display="none"; e.target.parentElement.innerHTML="◈"; }} />
          </span>
      ...
      {/* ChatBot */}
      {showChat && <ChatBot onClose={() => setShowChat(false)} allRestaurants={allRestaurants} />}
        ...
        <div style={{ position:"fixed", top:0, left:"50%", transform:"translateX(-50%)", width:"min(100vw, 480px)", bottom:0, zIndex:99999, height:"100%", background:C.cream, overflow:"hidden", isolation:"isolate" }}>
          ...
          <div style={{ height:"100%", overflowY:"auto", paddingBottom:40 }}>
            ...
            <div>
              ...
              {!detail.isBar && openTableDirectUrl && (
                <div style={{ padding: "20px 0", paddingLeft: 20, paddingRight: 20, borderBottom: `1px solid ${C.border}` }}>
                  <div style={{ fontFamily: "'DM Mono',monospace", fontSize: 10, color: C.muted, letterSpacing: "2px", marginBottom: 14 }}>RESERVATIONS</div>
                  <a href={openTableDirectUrl} target="_blank" rel="noopener noreferrer"
                    style={{ padding: "10px 20px", borderRadius: 12, background: "#DA3743", display: "inline-flex", alignItems: "center", textDecoration: "none" }}>
                    <img src="/OpenTable_logo.png" alt="Book on OpenTable" style={{ height: 22, filter: "brightness(0) invert(1)" }} />
                  </a>
                </div>
              )}
