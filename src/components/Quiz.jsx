    ...
    <div style={{ minHeight:"100vh", background:C.cream, fontFamily:"'DM Sans',sans-serif", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ opacity: transitioning ? 0 : 1, transform: transitioning ? "translateX(-20px)" : "translateX(0)", transition:"all 0.28s ease", flex:1, display:"flex", flexDirection:"column" }}>
        <div style={{ padding:"32px 24px 0" }}>
          <div style={{ fontFamily:"'DM Mono',monospace", fontSize:10, color:C.muted, letterSpacing:"2px", marginBottom:12 }}>STEP {step+1} OF {STEPS.length}</div>
          <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:30, fontWeight:700, lineHeight:1.2, color:C.text }}>{current.question}</div>
        ...
        {current.type === "slider" && (
          <div style={{ padding:"32px 24px 0", flex:1, display:"flex", flexDirection:"column" }}>
            <div style={{ fontFamily:"'Cormorant Garamond',serif", fontSize:72, fontWeight:700, fontStyle:"italic", color:C.terracotta, lineHeight:1, marginBottom:6 }}>{current.formatVal(sliderVal)}</div>
            <div style={{ display:"flex", justifyContent:"space-between", fontFamily:"'DM Mono',monospace", fontSize:10, color:C.muted, marginBottom:24 }}><span>$15</span><span>$300+</span></div>
            ...
            <div style={{ marginTop:20, padding:"14px 16px", background:C.card, borderRadius:12, border:`1px solid ${C.border}`, fontSize:13, color:C.muted, fontStyle:"italic", lineHeight:1.5 }}>
            ...
            <div style={{ marginTop:"auto", paddingTop:32 }}>
              <button onClick={() => advance({ tags: current.tags(sliderVal), value: sliderVal })}
                style={{ width:"100%", padding:16, border:"none", borderRadius:14, background:C.terracotta, color:"#fff", fontSize:15, fontWeight:600, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
                That's my vibe →
              </button>
            </div>
          </div>
